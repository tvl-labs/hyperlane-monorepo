    ...
    ISpecifiesInterchainSecurityModule
{
     ...
     */
    function setInterchainGasPaymaster(
        address _interchainGasPaymaster
     ...
     * @param _interchainGasPaymaster The address of the InterchainGasPaymaster contract.
     */
    function _setInterchainGasPaymaster(
        address _interchainGasPaymaster
    ) internal onlyContract(_interchainGasPaymaster) {
...
library TypeCasts {
    ...
    function coerceString(
        bytes32 _buf
    ) internal pure returns (string memory _newStr) {
        uint8 _slen = 0;
    ...
    Versioned
{
    ...
    function process(
        bytes calldata _metadata,
        bytes calldata _message
    ) external override nonReentrantAndNotPaused {
...
contract OwnableMulticall is OwnableUpgradeable {
    ...
    ) internal returns (bytes[] memory resolveCalls) {
...
abstract contract Router is HyperlaneConnectionClient, IMessageRecipient {
    ...
    function _mustHaveRemoteRouter(
        uint32 _domain
    ) internal view returns (bytes32 _router) {
...
contract InterchainAccountRouter is Router, IInterchainAccountRouter {
    ...
    function _getInterchainAccount(
        bytes32 salt
    ) internal view returns (address) {
